...
For OAuth in Next.js SSR apps:

- Start OAuth in a Server Action with `createAuthActions()` and
  `skipBrowserRedirect: true`.
- Store the returned PKCE verifier in an httpOnly cookie.
- Exchange `insforge_code` in a Route Handler with `exchangeOAuthCode()`.
- Use `@insforge/sdk/ssr/middleware` from Next.js 16 `proxy.ts` to refresh the
  session before Server Components render.
- Do not rely on browser callback auto-detection in SSR mode.
