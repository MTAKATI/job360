...
- Feature 07 adds authenticated `POST /api/resume/extract`: it downloads the current user-scoped PDF from private InsForge Storage, extracts text with `pdf-parse`, and sends the text server-side to Qwen via OpenRouter using a structured output schema matching `ProfileFormValues`.
- Feature 07 AI provider update: resume extraction now uses Qwen through OpenRouter (`OPENROUTER_API_KEY`, optional `OPENROUTER_BASE_URL`, and `OPENROUTER_RESUME_MODEL`, default `qwen/qwen3.6-flash`) because the local OpenAI key has no available quota. The app still uses the OpenAI SDK, but points it at OpenRouter's OpenAI-compatible API.
