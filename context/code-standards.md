...
- Navigation-only auth actions may redirect instead of returning a result.
  Keep SDK calls inside try/catch and call `redirect()` outside the try block.
...
- `@insforge/sdk` — InsForge client, including the `/ssr` entry point
